...
const AP_Param::GroupInfo AC_Spiral::var_info[] = {
    // @Param: RADIUS
    // @DisplayName: Circle Radius
    // @Description: Defines the radius of the circle the vehicle will fly when in Spiral flight mode
    // @Units: cm
    // @Range: 0 10000
    // @Increment: 100
    // @User: Standard
    AP_GROUPINFO("RADIUS",  0,  AC_Spiral, _radius, AC_CIRCLE_RADIUS_DEFAULT),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("RATE",    1, AC_Spiral, _rate,    AC_CIRCLE_RATE_DEFAULT),
...
void AC_Spiral::init()
{
    ...
    // set spiral center to circle_radius ahead of stopping point
...
/// update - update spiral controller
void AC_Spiral::update()
{
    ...
    if (!is_zero(_radius)) {
        // calculate target position
        Vector3f target;
        target.x = _center.x + (50 * _angle_total / M_PI) * cosf(-_angle);
        target.y = _center.y - (50 * _angle_total / M_PI) * sinf(-_angle);
        target.z = _pos_control.get_alt_target();
